  ...
  while (1)
    {
	    uint8_t incoming;
        // Riceve sblocco
        if (HAL_UART_Receive(&huart3, &incoming, 1, 10) == HAL_OK && incoming == 'U') {
            uint32_t startTime = HAL_GetTick();
            int sentCount = 0;

            while (HAL_GetTick() - startTime < 5000 && sentCount < 2) {
                bool currentButtonState = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0);

                if (currentButtonState && !lastButtonState) {
                	char sendVal = '1';  // inviamo sempre '1'
                	HAL_UART_Transmit(&huart3, (uint8_t*)&sendVal, 1, 100);
                    HAL_GPIO_TogglePin(GPIOE, GPIO_PIN_13);
                    HAL_Delay(100);
                    HAL_GPIO_TogglePin(GPIOE, GPIO_PIN_13);

                    sentCount++;
                    startTime = HAL_GetTick(); // resetta timer dopo invio
                }
